...
class DataPortalLandTrade( DataPortal ):

    # 실거래 데이터 조회
    def get_api_data(self, lawd_cd, deal_ymd):

        response = self.req_api('토지매매', lawd_cd, deal_ymd )
    ...
    def make_value_template( self, his_list ):
        ...
        for idx, his in enumerate(his_list):
            ...
            # ['거래금액', '거래면적', '거래유형', '구분', '년', '법정동', '시군구', '용도지역', '월', '일', '중개사소재지', '지목', '지번', '지역코드', '해제사유발생일', '해제여부'],
            value_template += "(" + f"""
                {his['거래금액'].replace(',', '')}, CAST(NULLIF('{his['거래유형']}', 'None') AS CHAR ), CAST(NULLIF('{his['구분']}', 'None') AS CHAR ), '{his['법정동']}',                
                '{his['시군구']}', {his['년']}, {his['월']}, {his['일']}, 
                {his['거래면적']}, '{his['용도지역']}', '{his['지번']}', '{his['지목']}', '{his['지역코드']}', 
                CAST(NULLIF('{his['중개사소재지']}', 'None') AS CHAR ), CAST(NULLIF('{his['해제사유발생일']}', 'None') AS CHAR ), CAST(NULLIF('{his['해제여부']}', 'None') AS CHAR )
            """.format( his=his ) + ")"
    ...
    def make_insert_template( self, his_list ):
        ...
        sql_template = """
        insert into land_trade
        (
          `price`, `ct_type`, `ct_type2`, `bubjungdong`, 
          `sido`, `year`, `month`, `day`,            
          `size`, `purpose`, `jibun`, `jimok`, `loc_cd`, 
          `trader_loc`, `cancel_date`, `cancel_yn`
        )
        VALUES
          {value_template}
